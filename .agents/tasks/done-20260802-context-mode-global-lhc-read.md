...
Из workspace `/home/admin/gptadmin` context-mode успешно обрабатывает
`/home/admin/.local/share/last-human-commit/current/common/agents/Reviewer.md`
...
- Глобальное host-правило `Read(...)` только для канонического дерева
  `/home/admin/.local/share/last-human-commit/current/**`.
- Проверка положительного и отрицательного canary через `ctx_execute_file`.

## Explicit exclusions

- Не разрешать чтение всего `/home/admin/**`, `.ssh`, credentials или
...
- 2026-08-02: Added exactly
  `Read(/home/admin/.local/share/last-human-commit/current/**)` to the

...
- Symptom: direct GPTAdmin target `admin-server-100-browseros` leaves `schema` permanently `running`; GPTAdmin ShellMCP call `mcp_tools(ref=BrowserOS)` completes with `error: EOF`.

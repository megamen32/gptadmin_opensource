# Install GPTAdmin on Mac mini 203.0.113.10

Status: complete
Original user request: «Установи на мак мини 203.0.113.10 гптадмин тоже»
Objective: Установить клиент GPTAdmin/ShellMCP на Mac mini 203.0.113.10, подключить его к существующему GPTAdmin Hub и обеспечить автозапуск.
...
Confirmed scope: Mac mini 203.0.113.10; пользовательская установка GPTAdmin/ShellMCP; сохранение существующей конфигурации машины; автозапуск; проверка через Hub.
...
- Timestamp: pending
  ...
  Evidence: pending
  Current user P0: GPTAdmin на Mac mini 203.0.113.10
...
- Timestamp: 2026-08-03
  ...
  Current user P0: безопасно сделать Mac mini 203.0.113.10 управляемым агентом GPTAdmin через существующий Hub
...
- Timestamp: 2026-08-03
  ...
  Evidence: raw user confirmation «Господи да он это он»; matching LAN MAC/mDNS; dedicated known-hosts updated to current key; strict SSH subsequently authenticated `admin` using the surviving Mac-specific key; live identity is `Macmini6,2`, macOS `15.7.8`, `x86_64`.
  Current user P0: установить GPTAdmin/ShellMCP на подтверждённый Mac mini 203.0.113.10 и доказать Hub E2E
...
- Timestamp: 2026-08-03
  ...
  Current user P0: установить GPTAdmin/ShellMCP на новый Mac mini 203.0.113.10, обеспечить автозапуск и доказать Hub→Mac E2E
...
Human selection: Пользователь прямо выбрал установку на 203.0.113.10 и затем подтвердил новый endpoint/host key фразой «Господи да он это он»; это выбирает YAGNI-путь точечной ротации dedicated known-hosts.
...
Blocked by: нет
Evidence: current host key `SHA256:tiXhDUkRVB7va5PooklfqEKkQQkaBFpqPp3V462s8z4`; strict SSH identity `mac-mini-2012.lan / Macmini6,2 / macOS 15.7.8 / x86_64 / admin`; binary build 141 commit `04980acfd3d6f9924177683dac86dee5727eb959`; Hub target `shell:mac-mini-2012.lan` online/approved; post-restart E2E job `648ac6fc4c7af3b6955569b237351ed0` completed with returncode 0.
...
Tests: plist `OK`; launchd restart PID `48009` → `48191`; после restart ровно один canonical process; Hub target online/approved; E2E job `648ac6fc4c7af3b6955569b237351ed0` вернул `Macmini6,2`, macOS `15.7.8`, `x86_64`, `admin`, Mach-O x86_64, returncode 0.

...
Diagnose the public ingress serving `your-subdomain.t.became.bezrabotnyi.com`, restore it to the intended release-145 Hub path, and prove external version, Custom GPT, and MCP behavior.
...
- Initial public DNS had three A-records: `203.0.113.10`, `212.192.31.128`, and `185.240.120.152`.
- Direct per-IP probe showed `203.0.113.10=145` after primary rollout, while VPN2 and VUSA returned stale `140`.
...
- VUSA FRP dashboard identified stale fallback proxy `gptadmin-failover-your-subdomain-server01`; it was removed through the loopback-only FRP control API and the fallback reclaim path was exercised.

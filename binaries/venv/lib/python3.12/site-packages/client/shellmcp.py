# DEPRECATED: legacy Python shellmcp compatibility implementation.
# Primary GPTAdmin shell transport is go-shellmcp / shellmcp-go-canary.
# Keep this file only for compatibility with old/source installs.
# shellmcp.py compatibility entrypoint
"""
DEPRECATED legacy Python shellmcp compatibility implementation.
Use go-shellmcp / shellmcp-go-canary for new installs.

Голый shell API без Docker, но с:
 • /exec                             – любая команда
 • heartbeat → HUB_URL (если задан)

Env:
  SHELLMCP_TOKEN     bearer-токен (def: CHANGE_ME)
  ...
  SHELLMCP_TRANSPORT websocket|webhook|polling|auto (def: webhook)
  SHELLMCP_URL       explicit externally reachable URL sent to hub
  SHELLMCP_NAME      explicit server name sent to hub
  SHELLMCP_PROXY_FOR/SHELLMCP_PROXY_VIA/SHELLMCP_BACKEND optional topology metadata
  SHELLMCP_AUTO_UPDATE       1/true to enable safe self-update loop (def: off)
  SHELLMCP_UPDATE_MANIFEST_URL manifest URL with build_version, sha256, url
  SHELLMCP_UPDATE_TOKEN optional Bearer token for update manifest/artifact
  SHELLMCP_UPDATE_INTERVAL_S update check interval (def: 3600)
"""
...
def _env(name: str, default: str | None = None) -> str | None:
    """New shell MCP env names with SHELLMCP_* compatibility fallback."""
    return os.getenv(f"SHELL_{name}", os.getenv(f"SHELLMCP_{name}", default))
...
log = logging.getLogger("shellmcp")
...
SHELLMCP_URL = _env("URL")
SHELLMCP_NAME = _env("NAME")
SHELLMCP_PROXY_FOR = _env("PROXY_FOR")
SHELLMCP_PROXY_VIA = _env("PROXY_VIA")
SHELLMCP_BACKEND = _env("BACKEND") or ("ssh" if os.getenv("SSH_HOST") else "local")
SHELLMCP_IDENTITY_DIR = _env("IDENTITY_DIR") or ("/etc/gptadmin" if os.access("/etc", os.W_OK) else str(Path.home() / ".gptadmin"))
HUB_PUBLIC_KEY_FILE = os.getenv("HUB_PUBLIC_KEY_FILE", str(Path(SHELLMCP_IDENTITY_DIR) / "hub_ed25519.pub"))
HUB_PUBLIC_KEY_B64 = os.getenv("HUB_PUBLIC_KEY", "")
SHELLMCP_IDENTITY = load_or_create_identity(SHELLMCP_IDENTITY_DIR, SHELLMCP_NAME or socket.gethostname(), prefix="shellmcp")
...
NONCES = NonceCache(ttl_s=int(os.getenv("SHELLMCP_NONCE_TTL_S", "300")))
TRANSPORT = _env("TRANSPORT", "webhook").lower()
HB_INT = int(os.getenv("HB_INTERVAL_S", "60"))
SHELLMCP_AUTO_UPDATE = _env("AUTO_UPDATE", "0").lower() in {"1", "true", "yes", "on"}
SHELLMCP_UPDATE_INTERVAL_S = int(_env("UPDATE_INTERVAL_S", "3600"))
SHELLMCP_SERVICE_NAME = _env("SERVICE_NAME", "shellmcp.service")
...
def _resolve_update_url(value: str | None, fallback: str | None = None) -> str:
    ...
    if parsed.scheme and parsed.netloc:
        return raw
    base = SHELLMCP_UPDATE_MANIFEST_URL or _hub_artifact_url("/") or HUB_URL
    if base and not base.endswith("/"):
        base = base.rsplit("/", 1)[0] + "/"
    return urljoin(base, raw)


SHELLMCP_UPDATE_MANIFEST_URL = _env("UPDATE_MANIFEST_URL") or _hub_artifact_url("/artifacts/shellmcp.json")
SHELLMCP_UPDATE_URL = _env("UPDATE_URL") or _hub_artifact_url("/artifacts/shellmcp.tar.gz")
SHELLMCP_UPDATE_TOKEN = _env("UPDATE_TOKEN", "")
SHELLMCP_SERVICE_SCOPE = _env("SERVICE_SCOPE", "system" if os.name != "nt" and hasattr(os, "geteuid") and os.geteuid() == 0 else "user").lower()
SHELLMCP_RESTART_CMD = _env("RESTART_CMD", "")
SHELLMCP_OUTBOX_DIR = Path(_env("OUTBOX_DIR", str(Path(SHELLMCP_IDENTITY_DIR) / "outbox")))
SHELLMCP_OUTBOX_RETRY_MIN_S = float(_env("OUTBOX_RETRY_MIN_S", "2"))
SHELLMCP_OUTBOX_RETRY_MAX_S = float(_env("OUTBOX_RETRY_MAX_S", "300"))
SHELLMCP_OUTBOX_SCAN_S = float(_env("OUTBOX_SCAN_S", "2"))
SHELLMCP_AUDIT_LOG = _env("AUDIT_LOG") or ("/var/log/gptadmin/shellmcp-audit.log" if os.access("/var/log", os.W_OK) else str(Path.home() / ".gptadmin" / "shellmcp-audit.log"))
try:
    _shellmcp_audit_path = Path(SHELLMCP_AUDIT_LOG)
    _shellmcp_audit_path.parent.mkdir(parents=True, exist_ok=True)
    _shellmcp_audit_handler = WatchedFileHandler(_shellmcp_audit_path)
    _shellmcp_audit_handler.setFormatter(logging.Formatter("%(message)s"))
    audit_log.addHandler(_shellmcp_audit_handler)
...
def _audit_event(event: dict) -> None:
    ...
    event.setdefault("server", SHELLMCP_NAME or socket.gethostname())
...
def _update_headers() -> dict:
    return {"Authorization": f"Bearer {SHELLMCP_UPDATE_TOKEN}"} if SHELLMCP_UPDATE_TOKEN else {}


def _signed_json_headers(method: str, path: str, body: bytes) -> dict:
    signed = sign_request(SHELLMCP_IDENTITY["private_key"], method, path, body)
    return {
        "Content-Type": "application/json",
        "X-GPTAdmin-Server": SHELLMCP_NAME or socket.gethostname(),
...
def _outbox_init() -> None:
    try:
        SHELLMCP_OUTBOX_DIR.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        log.warning("queue outbox disabled path=%s err=%s", SHELLMCP_OUTBOX_DIR, e)


def _outbox_metrics() -> dict:
    now = time.time()
    metrics = {
        "outbox_dir": str(SHELLMCP_OUTBOX_DIR),
        "outbox_files": 0,
        "outbox_bytes": 0,
    ...
    try:
        if not SHELLMCP_OUTBOX_DIR.exists():
            return metrics
        oldest = None
        last_error_at = 0
        for path in SHELLMCP_OUTBOX_DIR.glob("*.json"):
            try:
                st = path.stat()
...
def _outbox_file(message_id: str) -> Path:
    return SHELLMCP_OUTBOX_DIR / f"{message_id}.json"
...
def _outbox_backoff(path: Path, entry: dict, err: Exception) -> None:
    ...
    delay = min(SHELLMCP_OUTBOX_RETRY_MAX_S, SHELLMCP_OUTBOX_RETRY_MIN_S * (2 ** min(attempts - 1, 8)))
    entry.update({
        "attempts": attempts,
        "last_error": str(err),
...
def _queue_send_or_spool(srv_name: str, endpoint: str, payload: dict, kind: str, job_id: str, timeout: float = 5.0) -> None:
    ...
    try:
        ...
        _QUEUE_OFFLINE_UNTIL = time.time() + min(SHELLMCP_OUTBOX_RETRY_MAX_S, SHELLMCP_OUTBOX_RETRY_MIN_S * 2)
...
def outbox_retry_loop() -> None:
    ...
    while True:
        try:
            now = time.time()
            for path in sorted(SHELLMCP_OUTBOX_DIR.glob("*.json")):
                try:
                    entry = _outbox_load(path)
        ...
        time.sleep(SHELLMCP_OUTBOX_SCAN_S)

if os.getenv("QUEUE_URL"):
    QUEUE_URL = _derive_hub_queue_url(os.getenv("QUEUE_URL") or HUB_URL)
else:
    QUEUE_URL = None
CALLBACK_QUEUE_URL = os.getenv("SHELLMCP_CALLBACK_QUEUE_URL") or QUEUE_URL or _derive_hub_queue_url(HUB_URL)
...
app = FastAPI(title="shellmcp", version=str(BUILD_VERSION))
...
if os.getenv("SSH_HOST"):
    import shellmcp_ssh as backend
elif sys.platform.startswith("win"):
    import shellmcp_win as backend
elif sys.platform == "darwin":
    import shellmcp_mac as backend
else:
    import shellmcp_linux as backend
...
def _callback_job_runner(job_id: str, cmd: str, timeout: Optional[int], cwd: Optional[str], env: dict, request_source: str = "http_callback") -> None:
    srv_name = SHELLMCP_NAME or socket.gethostname()
...
def _mcp_supervisor_action(agent_id: str, action: str, *, backend: Optional[str] = None) -> dict:
    ...
    if result and result.get("returncode") is None and action != "status":
        ok = False
    return {"ok": ok, "action": action, "backend": backend, "service_name": name, "service_file": service_file, "before": before, "after": after, "result": result, "supervisor": "shellmcp_cross_platform_facade"}

def _mcp_capabilities(include_status: bool = True) -> list:
    cfg_path = Path(os.getenv("GPTADMIN_MCP_CONFIG", os.getenv("SHELLMCP_MCP_CONFIG", "/etc/gptadmin/mcp.json")))
    agents_dir = Path(os.getenv("GPTADMIN_MCP_AGENTS_DIR", os.getenv("SHELLMCP_MCP_AGENTS_DIR", "/etc/gptadmin/mcp-agents.d")))
    ...
    for name, spec in sorted(servers.items()):
        ...
        item = {
            ...
            "hosted_by": SHELLMCP_NAME or socket.gethostname(),
            "supervised_by": "shellmcp",
            ...
            "migration_state": "legacy_relay_supervised; shellmcp_registry_visible",
        }
        if include_status:
...
def _capability_registry(include_status: bool = True) -> dict:
    ...
    return {
        "ok": True,
        "schema_version": 1,
        "host": SHELLMCP_NAME or socket.gethostname(),
        "server_id": SHELLMCP_SERVER_ID,
        "transport_role": "shellmcp_transport_layer",
        "capability_host": True,
        "capabilities": [
            {"id": "shell", "kind": "shell", "role": "local_executor", "hosted_by": SHELLMCP_NAME or socket.gethostname()},
            {"id": "tasks", "kind": "task_store", "role": "durable_queue_view", "hosted_by": SHELLMCP_NAME or socket.gethostname()},
            {"id": "logs", "kind": "logs", "role": "diagnostics", "hosted_by": SHELLMCP_NAME or socket.gethostname()},
            {"id": "system", "kind": "system", "role": "host_introspection", "hosted_by": SHELLMCP_NAME or socket.gethostname()},
            *mcp,
        ],
...
def capabilities_mcp(include_status: bool = True):
    return {"ok": True, "host": SHELLMCP_NAME or socket.gethostname(), "capabilities": _mcp_capabilities(include_status=include_status)}
...
def _systemd_unit_action(unit: str, action: str) -> dict:
    ...
    if result and result.get("returncode") not in {0, None}:
        ok = False
    return {"ok": ok, "action": action, "unit": unit, "before": before, "after": after, "result": result, "supervisor": "shellmcp_systemd_facade"}
...
def capabilities_mcp_lifecycle(mcp_ref: str, payload: dict = Body(default_factory=dict)):
    ...
    res.update({"capability": cap, "host": SHELLMCP_NAME or socket.gethostname(), "server_id": SHELLMCP_SERVER_ID})
    return res
...
def _hub_ws_url() -> str:
    ...
    return urlunparse((scheme, parsed.netloc, "/ws/shellmcp", "", "", ""))
...
def _beat_payload(mode: str):
    try:
        ...
        info = {}
    return {
        "name": SHELLMCP_NAME or info.get("host", socket.gethostname()),
        "server_id": SHELLMCP_SERVER_ID,
        "public_key": SHELLMCP_PUBLIC_KEY_B64,
        "fingerprint": SHELLMCP_FINGERPRINT,
        "base_url": SHELLMCP_URL or f"http://{get_local_ip()}:{port}",
        ...
        "proxy_for": SHELLMCP_PROXY_FOR,
        "proxy_via": SHELLMCP_PROXY_VIA,
...
@app.get("/version")
def version():
    data = build_info("shellmcp")
    data.update({
        "transport": TRANSPORT,
        "deprecated": True,
        "deprecation_notice": "legacy Python shellmcp is deprecated; use go-shellmcp/shellmcp-go",
        "replacement": "go-shellmcp/shellmcp-go",
        ...
        "shellmcp_url": SHELLMCP_URL or f"http://{get_local_ip()}:{port}",
        "name": SHELLMCP_NAME or socket.gethostname(),
        "server_id": SHELLMCP_SERVER_ID,
        "fingerprint": SHELLMCP_FINGERPRINT,
        "public_key": SHELLMCP_PUBLIC_KEY_B64,
        "backend": SHELLMCP_BACKEND,
        ...
        "ssh_port": os.getenv("SSH_PORT"),
        "ssh_user": os.getenv("SSH_USER"),
        "auto_update": SHELLMCP_AUTO_UPDATE,
        "update_manifest_url": SHELLMCP_UPDATE_MANIFEST_URL if SHELLMCP_AUTO_UPDATE else None,
        **_outbox_metrics(),
    })
    return data
...
def _find_shellmcp_binary(extract_dir: str) -> str:
    candidates = [
        Path(extract_dir) / "shellmcp" / "dist" / "shellmcp",
        Path(extract_dir) / "build" / "shellmcp" / "dist" / "shellmcp",
        Path(extract_dir) / "shellmcp",
    ]
    for c in candidates:
        if c.is_file():
            return str(c)
    for c in Path(extract_dir).rglob("shellmcp"):
        if c.is_file() and c.stat().st_size > 1024 * 1024:
            return str(c)
    raise RuntimeError("shellmcp binary not found in update archive")
...
def _find_shellmcp_source_tree(extract_dir: str) -> Path:
    roots = [Path(extract_dir), Path(extract_dir) / "services" / "main_package" / "client", Path(extract_dir) / "client"]
    for root in roots:
        if (root / "shellmcp.py").is_file():
            return root
    for c in Path(extract_dir).rglob("shellmcp.py"):
        return c.parent
    raise RuntimeError("shellmcp.py source tree not found in update archive")


def _validate_source_tree(src: Path) -> None:
    required = ["shellmcp.py", "shellmcp_linux.py", "gptadmin_security.py", "gptadmin_build_info.py"]
...
def _canary_source_tree(src: Path) -> None:
    ...
    env = os.environ.copy()
    env.update({"PORT": port_probe, "SHELLMCP_PORT": port_probe, "HUB_URL": "", "SHELLMCP_AUTO_UPDATE": "0", "SHELLMCP_TRANSPORT": "webhook"})
    proc = subprocess.Popen([sys.executable, str(src / "shellmcp.py")], cwd=str(src), env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    try:
...
def _install_source_update(extract_dir: str, latest: int) -> dict:
    src = _find_shellmcp_source_tree(extract_dir)
    ...
    log.info("shellmcp source auto-update staged build %s over %s, backup=%s", latest, BUILD_VERSION, backup)
    subprocess.Popen(["/bin/sh", "-c", f"sleep 1; systemctl restart {SHELLMCP_SERVICE_NAME}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def _schedule_restart_after_update() -> None:
    if SHELLMCP_RESTART_CMD:
        cmd = f"sleep 1; {SHELLMCP_RESTART_CMD}"
    elif sys.platform == "darwin":
        label = SHELLMCP_SERVICE_NAME or "com.gptadmin.shellmcp"
        uid = os.getuid() if hasattr(os, "getuid") else 0
        if SHELLMCP_SERVICE_SCOPE == "user":
        ...
        if SHELLMCP_SERVICE_SCOPE == "user":
            cmd = f"sleep 1; systemctl --user restart {SHELLMCP_SERVICE_NAME} || kill -TERM {os.getpid()}"
        else:
            cmd = f"sleep 1; systemctl restart {SHELLMCP_SERVICE_NAME} || kill -TERM {os.getpid()}"
    subprocess.Popen(["/bin/sh", "-c", cmd], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def shellmcp_update_once() -> dict:
    if not SHELLMCP_UPDATE_MANIFEST_URL:
        return {"ok": False, "reason": "no manifest url"}
    manifest = requests.get(SHELLMCP_UPDATE_MANIFEST_URL, timeout=15, headers=_update_headers()).json()
    ...
    url = _resolve_update_url(manifest.get("url"), SHELLMCP_UPDATE_URL)
    ...
    with tempfile.TemporaryDirectory(prefix="shellmcp-update-") as td:
        archive = Path(td) / "shellmcp.tar.gz"
        ...
        if _is_source_install():
            raise RuntimeError("source-installed shellmcp cannot self-update from public artifacts; reinstall the binary package")
        new_bin = Path(_find_shellmcp_binary(str(extract_dir)))
    ...
    log.info("shellmcp binary auto-update installed build %s over %s, backup=%s", latest, BUILD_VERSION, backup)
...
def update_check():
    return shellmcp_update_once()


def auto_update_loop():
    if not SHELLMCP_AUTO_UPDATE:
        return
    while True:
        try:
            res = shellmcp_update_once()
            log.info("auto_update: %s", res)
        except Exception as e:
            log.warning("auto_update failed: %s", e)
        time.sleep(SHELLMCP_UPDATE_INTERVAL_S)


if SHELLMCP_AUTO_UPDATE:
...
def _polling_foreground_loop() -> None:
    log.info("shellmcp polling mode: HTTP listener disabled; waiting for queue jobs")
    while True:
        time.sleep(3600)
